    ...
    <>
      <Form noValidate className="py-1 border rounded-2" onSubmit={formik.handleSubmit} disabled={formik.isSubmitting}>
        <Form.Group className="has-validation input-group">
          <Form.Control disabled={formik.isSubmitting} isInvalid={!!error} name="body" aria-label={t('mainPage.messages.newMessage')} placeholder={t('mainPage.messages.enterMessage')} className="border-0 p-0 ps-2" ref={inputRef} value={formik.values.body} onChange={formik.handleChange} />
          ...
          <div class="invalid-tooltip">{t(`errors.${error}`)}</div>
        </Form.Group>
      </Form>
    </>
  );
